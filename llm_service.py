...
async def generate_questions(
    ...
    system_prompt = (
        ...
        "根據提供的法規框架與稽核範圍，產生一份稽核問題清單。\n\n"
        "輸出規則：\n"
        "- 僅輸出純 JSON array，不含 markdown、說明文字或其他內容\n"
        "- 每個元素格式：\n"
        '  {"id":"<uuid>","text":"<稽核問題>","category":"<稽核領域>","source_framework":"<法規名稱>","reference":"<條文或控制編號>"}\n'
        "- 稽核領域限用以下 10 種：治理與合規、風險管理、資產管理、存取控制、委外管理、事件應變、業務持續、情資威脅、教育訓練、隱私保護\n"
        "- 產生 15–25 題，涵蓋最相關的控制領域\n"
        "- 問題應具體、可回答，避免過於籠統\n"
        "- 每題 reference 欄位填入具體法規條文或控制編號（例如：資通安全管理法第15條、ISO 27001 A.5.19）"
    )
...
async def stream_findings(session: dict):
    ...
    json_schema = (
        ...
        '"regulatory_reference":"string","condition":"string",'
    ...
    system_prompt = "\n".join([
        ...
        "- condition：現況，稽核發現的事實",
        "- criteria：準則，適用法規或標準的具體要求",
        "- cause：原因，造成落差的根本原因",
        "- effect：影響，可能造成的風險或損害",
        "- recommendation：建議，具體改善措施含建議完成時程",
        "",
        "風險等級判定：",
        ...
        "- regulatory_reference 必須引用具體條文或控制編號",
        "- 發現依風險等級由高至低排序",
    ])
